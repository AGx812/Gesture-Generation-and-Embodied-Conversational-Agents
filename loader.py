...
class TedDBParams:
    def __init__(self, lmdb_dir, n_poses, subdivision_stride, pose_resampling_fps, mean_pose, mean_dir_vec,
        ...
        preloaded_dir = lmdb_dir + '_s2eg_v1_cache'
...
def load_ted_db_data(_path, dataset, config_args, ted_db_already_processed=False,
    ...
    if partition_data:
        lmdb_env = lmdb.open(j(_path, dataset, 'lmdb_train_tri_cache'), readonly=True, lock=False)
        map_size = 1024 * 50  # in MB
        map_size <<= 20  # in B
        lmdb_part_envs = [lmdb.open(j(_path, dataset, 'lmdb_train_s2eg_v1_cache'), map_size=map_size),
                          lmdb.open(j(_path, dataset, 'lmdb_eval_s2eg_v1_cache'), map_size=map_size),
                          lmdb.open(j(_path, dataset, 'lmdb_test_s2eg_v1_cache'), map_size=map_size)]
        num_samples = [0, 0, 0]
        k_idx = 0
        with lmdb_env.begin(write=False) as txn:
            for key, _ in txn.cursor():
    ...
    lmdb_part_envs = [lmdb.open(j(_path, dataset, 'lmdb_train_s2eg_v1_cache'), readonly=True, lock=False),
                      lmdb.open(j(_path, dataset, 'lmdb_eval_s2eg_v1_cache'), readonly=True, lock=False),
                      lmdb.open(j(_path, dataset, 'lmdb_test_s2eg_v1_cache'), readonly=True, lock=False)]
