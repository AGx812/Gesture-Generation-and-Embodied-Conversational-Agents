...
class Processor(object):
    ...
    def __init__(self, args, s2ag_config_args, data_loader, pose_dim, coords,
        ...
        # print(self.num_train_samples, self.num_eval_samples, self.num_test_samples)
        # print(self.args.train_s2ag)
        ...
        # print(self.args.train_s2ag, self.args.data_path, self.args.dataset_s2ag)
        # exit()
    ...
    def save_cache(self, part, file_name):
        ...
        mfcc_features_all = np.zeros((num_samples, 37, self.mfcc_length))
    ...
    def load_model_at_epoch(self, epoch='best'):
        ...
        model_name = "epoch_290_loss_-0.0048_model.pth.tar"
        model_found = False
        try:
    ...
    def generate_gestures_by_dataset(self, dataset, data_params, check_duration=True,
        ...
        if load_saved_model:
            ...
            assert s2ag_model_found, print('Speech to emotive gestures model not found')
            trimodal_checkpoint = torch.load(jn(base_path, 'outputs', 'trimodal_gen.pth.tar'))
            # trimodal_checkpoint = torch.load(jn(base_path, 'outputs', 'multimodal_context_checkpoint_best.bin'))
            self.trimodal_generator.load_state_dict(trimodal_checkpoint['trimodal_gen_dict'])
